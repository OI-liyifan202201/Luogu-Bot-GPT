...
def report(uid):
    ...
    try:
        ...
        memory.append(
            {
                'role': 'assistant',
                'content': 'I know it,OK.',
            }
        )
        memory.append(
            {
                'role': 'system',
                'content': 'Tips:'+tips.replace('{username}', user(uid)['name']),
            }
        )
        memory = memory[::-1]
        client = openai.OpenAI(
            api_key=ai_key, base_url='https://free.gpt.ge/v1/'
        )


        log.info(f'Get message')
        try:
            log.info("Use the GPT-4o")
            ...
            try:
                log.warning(f'GPT-4o模型访问量过大，自动切换成为GPT-4-turbo')
                response = client.chat.completions.create(
                    model='gpt-4-turbo',
                    messages=memory,
                    stream=True,
                ...
                log.warning(f'GPT-4-turbo模型访问量过大，自动切换成为GPT-3.5-turbo')
                response = client.chat.completions.create(
        ...
        log.success(f'Send txt to Open AI')
        temp = ''
        reason = None
        for data in response:
            if data.choices[0].delta.content == None:
                break
